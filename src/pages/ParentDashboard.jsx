...
export default function ParentDashboard() {
  ...
  // load server children (when parent identity becomes available)
  useEffect(() => {
    let ignore = false;
    async function run() {
  ...
  const selectedChildEmail = selectedChild?.email || "";
  const selectedChildName = selectedChild?.name || "";

  useEffect(() => {
    setEditName(selectedChildName);
  }, [selectedChildEmail, selectedChildName]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        if (!ignore) setChildProfile(null);
      }
    }
    run();
    return () => { ignore = true; };
    ...
    <div style={styles.wrap}>
      ...
      <div style={styles.headerRow}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Link to="/leaderboard/parents" style={styles.secondaryBtn}>Parents Leaderboard</Link>
          <Link to="/leaderboard/schools" style={styles.secondaryBtn}>Schools Leaderboard</Link>
        </div>
      </div>
              ...
              return (
                <div key={c.email} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <button
                    ...
                    aria-label={`Remove ${c.name || "Student"}`}
                  >
                    ×
                  </button>
                </div>
      ...
      <section style={styles.focusHero}>
        <div style={{ display: "grid", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            ...
            <Link to="/career/results" style={styles.pillBtnOutline}>Assessment Results</Link>
          </div>
        </div>
              ...
              <>
                ...
                {childFocus ? (
                  ...
                  <div style={styles.widgetHint}>
                    ...
                    <Link to="/career" style={styles.widgetInlineLink}>Career Assessment</Link>.
                  </div>
                )}
              </>
            ) : (
      ...
      <section>
        <div style={styles.rowHeader}>
          ...
          <Link to="/career/results" style={styles.link}>See results</Link>
        </div>
