...
function SidebarLayout({ children }) {
  ...
  // Plain CSS fallback via inline styles so it works even if utilities are overridden
  const linkBaseStyle = {
    display: "flex",
    alignItems: "center",
    gap: 12,
    ...
    fontSize: 13,
    fontWeight: 600,
    boxShadow: "0 1px 1px rgba(2,6,23,0.04)",
  };
  ...
  return (
    <div className="relative min-h-screen" data-layout="fp-new">
      ...
      <aside
        ...
        aria-label="Primary"
        style={{ position: "fixed", top: 0, left: 0, height: "100vh", width: 256 }}
      >
        {/* Logo */}
        ...
        {/* Vertical menu — real button containers */}
        <nav className="flex flex-col gap-2" data-nav="fp-vertical" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <NavLink to="/student" className="no-underline"
            style={({ isActive }) => ({ ...linkBaseStyle, ...(isActive ? linkActiveStyle : null) })}>
            <IconDashboard /> <span className="truncate">Dashboard</span>
          </NavLink>

          <NavLink to="/tasks" className="no-underline"
            style={({ isActive }) => ({ ...linkBaseStyle, ...(isActive ? linkActiveStyle : null) })}>
            <IconTasks /> <span className="truncate">Tasks</span>
          </NavLink>

          <NavLink to="/mentors" className="no-underline"
            style={({ isActive }) => ({ ...linkBaseStyle, ...(isActive ? linkActiveStyle : null) })}>
          ...
          <NavLink to="/journal" className="no-underline"
            style={({ isActive }) => ({ ...linkBaseStyle, ...(isActive ? linkActiveStyle : null) })}>
          ...
          <NavLink to="/resources" className="no-underline"
            style={({ isActive }) => ({ ...linkBaseStyle, ...(isActive ? linkActiveStyle : null) })}>
            <IconResources /> <span className="truncate">Resources</span>
          </NavLink>

          <NavLink to="/bookmarks" className="no-underline"
            style={({ isActive }) => ({ ...linkBaseStyle, ...(isActive ? linkActiveStyle : null) })}>
      ...
      {/* TOP-RIGHT: compact user card — hard-pinned */}
      <div
        data-rail="fp-user-right-card"
        className="rounded-2xl border bg-white/95 p-4 shadow-lg backdrop-blur"
        style={{ position: "fixed", right: 16, top: 16, zIndex: 1000, width: 320 }}
      >
        <UserRail />
      </div>

      {/* CENTER content — keep padding so the fixed pieces don't overlap */}
      <div className="content-shell" style={{ paddingLeft: 256, paddingRight: 380 }}>
  ...
  return (
    <div>
      ...
      <div className="mt-3 grid gap-2">
        <NavLink to="/bookmarks" className="rounded-md border px-3 py-2 text-sm hover:bg-gray-50 no-underline">
          View Bookmarks
        </NavLink>
        <NavLink to="/career/results" className="rounded-md border px-3 py-2 text-sm hover:bg-gray-50 no-underline">
...
function Home() {
  return (
    <div className="space-y-3">
      ...
      <p className="text-gray-700">Welcome. Use the navigation to explore dashboards, resources, and tools.</p>
    </div>
  );
}
